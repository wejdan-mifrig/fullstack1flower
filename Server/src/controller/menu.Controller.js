...
export const updateMenuItemController = asyncHandler(async (req, res) => {
  ...
  const currentItem = await pool.query("SELECT * FROM menu WHERE id = $1", [
    id,
  ]);
  ...
  let imageName = currentItem.rows[0].image;
    ...
    if (currentItem.rows[0].image) {
      ...
      if (fs.existsSync(oldImagePath)) {
        try {
          ...
          console.log(" Old image deleted:", currentItem.rows[0].image);
        } catch (err) {
  ...
  const updatedItem = await updateMenuItem(id, {
    ...
    price:
      price !== undefined && price !== ""
        ? parseFloat(price)
        : currentItem.rows[0].price,
    image: imageName,
    category_id: category_id
      ? parseInt(category_id)
      : currentItem.rows[0].category_id,
  });
  ...
  if (currentItem.rows[0].image) {
    ...
    if (fs.existsSync(imagePath)) {
      try {
        ...
        console.log(" Image deleted:", currentItem.rows[0].image);
      } catch (err) {
