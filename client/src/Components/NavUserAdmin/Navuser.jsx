...
export default function NavbarUser() {
  ...
  const { user, authorizedUser } = useContext(UserContext);
  ...
  return (
    <>
      ...
      >
        <Toolbar sx={{ justifyContent: "space-between", px: 5, py: 2 }}>
      ...
      >
        <Box sx={{ p: 4, display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          <Typography sx={{ fontSize: ".7rem", color: "#999", mb: 1, letterSpacing: 2 }}>
            NAVIGATION
          </Typography>
          ...
          {navItems.map((item, index) => (
            ...
            >
              ...
              >
                <Typography sx={{ fontSize: ".9rem", letterSpacing: 1 }}>
                  {item.label}
                </Typography>
          ...
          {user && user.name && (
            <Box sx={{ mb: 2 }}>
              {["MY PROFILE", "MY BOOKINGS", "CART"].map((text, i) => (
                <Typography
                  key={i}
                  onClick={() =>
                    handleNav(
